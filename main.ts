	...
	async sync(): void {
		...
		for (let new_note of temp_new) {
			...
			for (let linked_file of links[new_note[0].path]) {
				let q = this.scheduled_notes[linked_file[0]];
				if (q) {
...
class DueDatesListView extends ItemView {
	...
	public redraw(): void {
		...
		for (let currentFile in this.plugin.scheduled_notes) {
			currentFile = this.plugin.scheduled_notes[currentFile];
			...
			let due = new Date(currentFile[1]);
			navFileTitle.createDiv({
				cls: 'nav-file-title-content',
				text: `${currentFile[0].basename} - ${due.toDateString()}` 
			});
