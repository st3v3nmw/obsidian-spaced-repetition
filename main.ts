...
export default class NotesReviewPlugin extends Plugin {
	...
	async onload() {
		...
		this.app.workspace.on("file-menu", ((menu, file: TFile) => {
			menu.addItem((item) =>{
				item.setTitle("Note Review: Easy").setIcon("crosshairs")
					.onClick(evt => {
		...
		this.app.workspace.on("file-menu", ((menu, file: TFile) => {
			menu.addItem((item) =>{
				item.setTitle("Note Review: Hard").setIcon("crosshairs")
					.onClick(evt => {
	...
	async onunload() {
		console.log("unloading Notes Review plugin...");
	}
	...
	async sync() {
		...
		for (let note of notes) {
			...
			if (!IGNORE_REGEX.test(file_text)) { // checks if note should be ignored
				let yaml_header = /^---\n((?:.*\n)*)---/.exec(file_text);
				let yaml_header_length = (yaml_header == null ? 0 : yaml_header[0].split(/\r\n|\r|\n/).length);
				if (file_text.split(/\r\n|\r|\n/).length - yaml_header_length >= 3) { // file should have more than 3 or more lines
		...
		for (let new_note of temp_new) {
			...
			let initial_ease = 100;			
			if (outgoing_link_count > 0 && incoming_link_count > 0)
				initial_ease += Math.floor(0.4 * outgoing_link_total / outgoing_link_count + 0.2 * incoming_link_total / incoming_link_count);
			else if (outgoing_link_count > 0)
				initial_ease += Math.floor(0.6 * outgoing_link_total / outgoing_link_count);
			else if (incoming_link_count > 0)
				initial_ease += Math.floor(0.6 * incoming_link_total / incoming_link_count);
			else
				initial_ease += 150;
	...
	async saveReviewResponse(note: TFile, quality: Number) {
		...
		if (!IGNORE_REGEX.test(file_text)) { // checks if note should be ignored 
			if (file_text.split(/\r\n|\r|\n/).length >= 4) { // file should have more than 3 or more lines
				...
				new Notice("Note not scheduled yet.");
			}
		} else {
