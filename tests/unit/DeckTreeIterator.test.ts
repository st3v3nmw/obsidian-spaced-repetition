...
import { CardListOrder, DeckTreeSequentialIterator, IIteratorOrder, OrderMethod } from "src/DeckTreeIterator";
...
describe("setDeck", () => {
    test("currentDeck null immediately after setDeck", async () => {
        ...
        let iterator: DeckTreeSequentialIterator = new DeckTreeSequentialIterator(order_NewFirst_Sequential);
...
describe("nextCard", () => {
    describe("Due cards before new cards, sequential ordering", () => {
        test("Single topic, new cards only", async () => {
            ...
            Q3::A3`;
            let deck: Deck = await SampleItemDecks.createDeckFromText(text, new TopicPath(["Root"]));
            let iterator: DeckTreeSequentialIterator = new DeckTreeSequentialIterator(order_DueFirst_Sequential);
            iterator.setDeck(deck);
        ...
        describe("Single topic, mixture of new and scheduled cards", () => {
            test("Get the scheduled cards first", async () => {
                ...
                Q6::A6`;
                let deck: Deck = await SampleItemDecks.createDeckFromText(text, new TopicPath(["Root"]));
                iterator = new DeckTreeSequentialIterator(order_DueFirst_Sequential);
                iterator.setDeck(deck);
    ...
    describe("New cards before due cards, sequential ordering", () => {
        test("Single topic, new cards only", async () => {
            let text: string = `
            Q1::A1
            Q2::A2
