...
const execInlineCardsTestWithSeparator = (separator: string, separatorCardType: CardType) => {
    ...
    expect(parseT([
        `#flashcards/science Question1${separator}Answer1`,
    ].join("\n"),)).toEqual([[separatorCardType, [
        `#flashcards/science Question1${separator}Answer1`,
    ].join("\n"), 0, 0],
    ]);
    ...
    expect(parseT([
        "#flashcards/science",
        `Question${separator}Answer`
    ].join("\n"), parserOptions)).toEqual([[separatorCardType, [
        `Question${separator}Answer`
    ].join("\n"), 1, 1],
    ]);

    expect(parseT([
        "#flashcards/science #flashcards/math",
        `Question${separator}Answer`
    ].join("\n"), parserOptions)).toEqual([[separatorCardType, [
        `Question${separator}Answer`
    ].join("\n"), 1, 1],
...
test("Test parsing of single line basic cards", () => {
    ...
    ).toEqual([[CardType.SingleLineBasic,
        "Question=Answer",
        0, 0
    ]]);
...
test("Test parsing of single line reversed cards", () => {
    ...
    ).toEqual([[CardType.SingleLineReversed,
        "Question==Answer",
        0, 0
    ]]);
