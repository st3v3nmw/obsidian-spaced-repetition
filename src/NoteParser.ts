...
export class NoteParser {
    ...
    async parse(noteFile: ISRFile, defaultTextDirection: TextDirection, folderTopicPath: TopicPath): Promise<Note> {
        const questionParser: NoteQuestionParser = new NoteQuestionParser(this.settings);
        const questions = await questionParser.createQuestionList(noteFile, defaultTextDirection, folderTopicPath, true);
