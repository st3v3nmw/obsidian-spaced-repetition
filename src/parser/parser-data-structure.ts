...
export class ParsedQuestionInfo {
    ...
    constructor(cardType: CardType, text: string, firstLineNum: number, lastLineNum: number, frontText: string | null = null, backText: string | null = null) {
...
export class CardData {
     ...
     */
    initNewPotentialCard(type: CardType, text: string, firstLineNum: number, lastLineNum: number, frontText: string | null = null, backText: string | null = null) {
    ...
    addCurrentLineToLastCard(lineData: LineData, searchForMultilineCards: boolean = false): void {
        console.log("addCurrentLineToLastCard", this.cards);
        if (this.lastCardIndex < 0) {
            return;
        }
    ...
    addStartOfMultilineCardToList(lineData: LineData): void {
        ...
        for (const separator of multilineSeparators) {
            if (lineData.currentLineTrimmed.startsWith(separator.separator)) {
    ...
    */
    addInlineCardToList(lineData: LineData, notePath: string, noteText: string): void {
        ...
        for (const { separator, type } of inlineSeparators) {
            if (// We have found an inline card separator
                ...
                break;
            } else if (QuestionParser.hasInlineSeparator(lineData.currentLineTrimmed, [separator])) {
    ...
    addClozeToList(lineData: LineData): void {
        ...
        if (this.potentialNewCard !== null && this.potentialNewCard.text.length > 0) {
            this.cards[this.lastCardIndex].text = this.potentialNewCard.text + "\n" + this.cards[this.lastCardIndex].text;
